...
| 推論當事實講、同文件取捨判準前後不一致，被抓到一次只修那一句，同類問題留給 user 逐條抓 | `hooks/hooks.json`（PostToolUse／Write\|Edit）派獨立 agent 審剛寫入的內容，抓無依據的肯定陳述與判準不一致，不靠模型自己重讀；每次 Write/Edit 都審，非只在被抓到後才補查 |
