...
**寫行為，不寫裁決論述** — 業務規則描述系統做什麼、產生什麼結果（可直接轉譯成程式邏輯的動作句），不寫「如何仲裁/認定/效力」這類論述——論述需要讀者自行翻譯，工程師讀完不知道要寫什麼。

**先消滅衝突，不急著仲裁** — 規則間出現狀態衝突時，先問能不能重新設計狀態讓衝突不存在（如：觸發當下所有相關狀態一併收斂），能消滅就消滅；不能消滅才用優先權/例外規則管理。能消滅卻選擇管理 → 規則要求每個讀取方各自記住特例，容易漏、忘、不同步。

**不綁入口** — 規則描述動作本身的效果，不夾帶「從哪個入口觸發」的細節；入口/時序由流程章節（§三流程等）管，兩者分開寫。

**零新名詞** — 規則全篇只用文件既有語彙（既有欄位名/狀態名/動作名），不因為要表達新概念就發明新詞彙。
