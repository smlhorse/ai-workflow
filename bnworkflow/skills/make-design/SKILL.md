...
**獨立 facet 平行產出** — 觸發的 facet 間無依賴時（如系統架構/軟體架構/infra/介面互動各自獨立），各派對應角色 agent 平行產出，不得一人全包；有依賴的 facet（如威脅模型需先有系統架構的攻擊面）待前置 facet 完成才動手。
...
- **系統架構**（系統架構師）：服務切分（預設前後端分離，純後端 service/CLI/library 等專案類型須於專案 CLAUDE.md「架構摘要」明確排除才可跳過）、整合方式、資料一致性、容錯、容量/SLA
