...
此 repo 是 Codex plugin 來源。對外以 marketplace 形式發布（`.claude-plugin/marketplace.json`），實際 plugin 在 `bnworkflow/`。以下規範適用於維護、新增、修改任何 skill。
...
規則內容只有一份、住 `bnworkflow/skills/init/templates/rules.md`（＋`templates/roles.md`）——這點沒變。差別是：框架自身的 `.claude/CLAUDE.md` / `.claude/roles.md`（Codex 在本 repo 的**自動載入入口，仍必須存在**）現在**只 `@import` 那一份、不再各存一份手動同步**。改規則只改 templates。init 給用戶專案的是 templates 的**全文複本**（用戶端無 templates 可 import）。
...
│       └── ...（共 27 個 skill；部分 skill 含 templates/）
├── .claude/                          ← 框架自身運行規則（維護者用，Codex 共用讀取）
│   ├── CLAUDE.md
│   ├── roles.md
│   └── settings.json
├── AGENTS.md                         ← 本檔，維護說明（不進 plugin）
├── README.md
└── .gitignore
```
